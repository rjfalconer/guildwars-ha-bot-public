...
Local $CallSkillHeader = 0x2C
Local $AttackHeader = 0x2C ; DUPE
Local $CancelActionHeader = 0x2E
Local $DropBuffHeader = 0x2F
Local $DrawMapHeader = 0x31
...
Local $MouseMoveHeader = 0x44
Local $PickUpItemHeader = 0x45
Local $ChangeSecondProfessionHeader = 0x47
Local $UseSkillHeader = 0x4C
Local $StartTradeHeader = 0x4F
Local $TransactTradeHeader = 0x50
Local $RequestQuoteHeader = 0x52
Local $TransactItemHeader = 0x53
Local $GoSignpostHeader = 0x57 ; "GoGadget"
...
Local $DeleteItemHeader = 0x6E
Local $IdentifyItemHeader = 0x71
Local $UnlockSkillByTomeHeader = 0x72
Local $MoveItemHeader = 0x77
Local $AcceptAllItemsHeader = 0x78
Local $SplitStackHeader = 0x7A
Local $StartSalvageHeader = 0x7C
Local $SalvageMaterialsHeader = 0x7F
Local $SalvageModHeader = 0x80
Local $ChangeGoldHeader = 0x81
Local $UseItemHeader = 0x83
Local $RequestSpawnHeader = 0x8D
Local $RequestAgentsHeader = 0x96
Local $RequestItemsHeader = 0x97
Local $SwitchModeHeader = 0xA1
Local $DenyPartyRequestHeader = 0xA2
Local $AcceptPartyRequestHeader = 0xA4
Local $AddNpcHeader = 0xA5
Local $InvitePlayerHeader = 0xA6;
Local $InvitePlayerByNameHeader = 0xA7;
Local $LeaveGroupHeader = 0xA8
Local $EnterChallengeHeader = 0xAB
Local $ReturnToOutpostHeader = 0xAD
Local $KickNpcHeader = 0xAE
Local $KickPlayerHeader = 0xAF
Local $TickHeader = 0xB5
Local $TravelGHHeader = 0xB6
Local $MoveMapHeader = 0xB7
Local $LeaveGHHeader = 0xB8
Local $AcceptTradeHeader = 0x7
Local $AcknowledgeTradeHeader = 0x0
Local $AddItemTradeHeader = 0x2
Local $CancelTradeHeader = 0x1
Local $ChangeOfferHeader = 0x6
Local $RemoveItemTradeHeader = 0x5
Local $SubmitOfferHeader = 0x3
#EndRegion Headers
...
Func GetLoggedCharNames()
	Local $array = scangw()
	If $array[0] <= 0 Then
		;Out("Could not find any characters")
		Return ""
	EndIf
	Local $ret = $array[1]
	For $i = 2 To $array[0]
		$ret &= Opt("GUIDataSeparatorChar")
		$ret &= $array[$i]
	Next
	Return $ret
EndFunc
...
Func ScanGW()
	Local $lwinlist = WinList("Guild War") ; My patcher replaces the last character with a number
	Local $lreturnarray[1] = [0]
	Local $lpid
	For $i = 1 To $lwinlist[0][0]
		ConsoleWrite("Looking for process with title starting 'Guild War'... ")
		$mgwhwnd = $lwinlist[$i][1]
		$lpid = WinGetProcess($mgwhwnd)
		Local $processName = GetProcessName($lpid)
		If StringLeft($processName, 2) <> "gw" Then ; aaand I number the exes Gw1.exe, Gw2.exe, etc
			ConsoleWrite($processName & " does not start with Gw" & @CRLF)
			ContinueLoop
		EndIf
		ConsoleWrite($processName & "... ")
		memoryopen(WinGetProcess($mgwhwnd))
		If $mgwprochandle Then
			$lreturnarray[0] += 1
			ReDim $lreturnarray[$lreturnarray[0] + 1]
			Local $charName = scanforcharname()
			ConsoleWrite($charName & @CRLF)
			$lreturnarray[$lreturnarray[0]] = $charName
		Else
...
Func DestroyItem($aItem)
	...
	Return SendPacket(0x8, $DeleteItemHeader, $lItemID)
EndFunc   ;==>DestroyItem
...
Func TradePlayer($aAgent)
	...
	EndIf
	SendPacket(8, $StartTradeHeader, $lAgentID)
EndFunc

Func AcceptTrade()
	Return SendPacket(4, $AcceptTradeHeader)
EndFunc

Func SubmitOffer($aGold = 0)
	Return SendPacket(8, $SubmitOfferHeader, $aGold)
EndFunc

Func CancelTrade()
	Return SendPacket(4, $CancelTradeHeader)
EndFunc

Func ChangeOffer()
	Return SendPacket(4, $ChangeOfferHeader)
EndFunc

Func OfferItem($lItemid, $aQuantity = 1)
	Return SendPacket(12, $AddItemTradeHeader, $lItemid, $aQuantity)
EndFunc
...
Func MoveTo($aX, $aY, $aRandom = 50)
	...
	While ComputeDistance(DllStructGetData($lMe, 'X'), DllStructGetData($lMe, 'Y'), $lDestX, $lDestY) >= 25
		Sleep(100)
		$lMe = GetAgentByID(-2)
...
;~ Description: Talks to an NPC and waits until you reach them.
Func GoToNPC($aAgent)
	...
	Sleep(100)
	GoNPC($aAgent)
	Sleep(GetPing() + 300) ; Wait for your character to have a chance to start moving

	Local $lBlockedMax = 15
	Do
		Sleep(100)
		$lMe = GetAgentByID(-2)
		...
		If DllStructGetData($lMe, 'MoveX') == 0 And DllStructGetData($lMe, 'MoveY') == 0 Then
			If GetDistance($lMe, $aAgent) > 170 Then ; Adjacent
				$lBlocked += 1
				Move(DllStructGetData($aAgent, 'X'), DllStructGetData($aAgent, 'Y'), 100)
				Sleep(100)
				GoNPC($aAgent)
			EndIf
		EndIf
	Until ComputeDistance(DllStructGetData($lMe, 'X'), DllStructGetData($lMe, 'Y'), DllStructGetData($aAgent, 'X'), DllStructGetData($aAgent, 'Y')) < 250 Or $lBlocked > $lBlockedMax
	Sleep(GetPing() + Random(500, 1000, 1))

	If $lBlocked >= $lBlockedMax Then
		Return False
	Else
		SetError($lBlocked) ; Allow calling code to query how often agent was blocked
		Return True
	EndIf
EndFunc   ;==>GoToNPC
...
Func DropBuff($aSkillID, $aAgentID, $aHeroNumber = 0)
	Local $lBuffStruct = DllStructCreate($gBuffStruct)
...
Func DonateFaction($aFaction)
	If StringLeft($aFaction, 1) = 'k' Then
		Return SendPacket(0x10, $DonateFactionHeader, 0, 0, 5000)
	ElseIf StringLeft($aFaction, 1) = 'l' Then
		Return SendPacket(0x10, $DonateFactionHeader, 0, 1, 5000)
	EndIf
...
Func GetAgentByID($aAgentID = -2)
	...
	Local $lAgentStruct = DllStructCreate($gAgentStruct)
...
Func GetMovStructByID($aAgentID = -2)
	Local $lAgentMovPtr = GetMovementAgentPtr($aAgentID)
	If $lAgentMovPtr = 0 Then Return 0
	Local $lAgentMovStruct = DllStructCreate('ptr vtable;byte unknown1[12];long AgentID;byte unknown2[40];long Moving1;byte unknown3[8];long Moving2;byte unknown4[28];float X2;float Y2;byte unknown5[8];float X;float Y')
	DllCall($mKernelHandle, 'int', 'ReadProcessMemory', 'int', $mGWProcHandle, 'int', $lAgentMovPtr, 'ptr', DllStructGetPtr($lAgentMovStruct), 'int', DllStructGetSize($lAgentMovStruct), 'int', '')
	Return $lAgentMovStruct
EndFunc

Func GetMovementAgentPtr($aAgentID)
	Local $lOffset[] = [0, 0x18, 0x8, 0xE8]
	Local $lAgentStructAddress = MemoryReadPtr($mBasePointer, $lOffset)
	Local $lOffset2[3] = [0, 4 * ConvertID($aAgentID), 0]
	Local $lAgent2StructAddress = MemoryReadPtr($lAgentStructAddress[0], $lOffset2)
	Return $lAgent2StructAddress[0]
EndFunc
...
Func GetParty($aAgentArray = 0)
	...
	For $i = 1 To $aAgentArray[0]
		If DllStructGetData($aAgentArray[$i], 'Allegiance') == 1 Then
			; 0x20000; CanBeViewedInPartyWindow
			If BitAND(DllStructGetData($aAgentArray[$i], 'TypeMap'), 0x20000) Then
				$lReturnArray[0] += 1
				ReDim $lReturnArray[$lReturnArray[0] + 1]
...
Func GetAgentArray($aType = 0)
	...
	For $i = 1 To $lCount
		$lReturnArray[$i] = DllStructCreate($gAgentStruct)
...
Func GetBuffByIndex($aBuffNumber, $aHeroNumber = 0)
	...
	For $i = 0 To $lCount[1] - 1
		...
		If $lBuffer[1] == GetHeroID($aHeroNumber) Then
			...
			Local $lBuffStructAddress = MemoryReadPtr($mBasePointer, $lOffset)
			DllCall($mKernelHandle, 'int', 'ReadProcessMemory', 'int', $mGWProcHandle, 'int', $lBuffStructAddress[0], 'ptr', DllStructGetPtr($lBuffStruct), 'int', DllStructGetSize($lBuffStruct), 'int', '')
			Return $lBuffStruct
		EndIf
	Next
		...
		If $lBuffer[1] == GetHeroID($aHeroNumber) Then
			...
			If $aSkillID = 0 Then
				...
				For $i = 0 To $lEffectCount[1] - 1
					$lReturnArray[$i + 1] = DllStructCreate($gEffectStruct)
...
;~ 	Waits $aDeadlock for load to start, and 500ms for agent to load after map is loaded.
	Local $lMapLoading
	Local $lDeadlock = TimerInit()
	...
	Until $lMapLoading <> 2 And GetMapIsLoaded() And (GetMapID() = $aMapID Or $aMapID = 0) ;And GetAgentExists(GetMyId())
...
Func GetQuestByID($aQuestID = 0)
	Local $lQuestStruct = DllStructCreate($gQuestStruct)
